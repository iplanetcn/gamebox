...
class ChessBoard(
    ...
    private var chessFactory: ChessFactory
    ...
    private val chessMap: MutableMap<String, ChessType>
    ...
    fun renderBoard(shapeRenderer: ShapeRenderer) {
        ...
        // draw chess pieces
        chessMap.forEach {
            chessFactory.getChess(it.value).draw(shapeRenderer, gridBoxMap[it.key]!!.pos, gridSize)
        }
    ...
    /**
     * add Step To ChessMap
     */
    fun addStepToChessMap(step: Step) {
        chessMap[generateKeyByIndex(step.index)] = step.chessType
    }
}